...
function quoteItemRow(data={}){
 ...
 wrap.querySelector('.remove-quote-item').onclick=()=>{if(document.querySelectorAll('.quote-item').length<=1)return alert('At least one item is required.');wrap.remove();renumberQuoteItems();calcTotal()};
 wrap.querySelector('.duplicate-quote-item').onclick=()=>{const d=getQuoteItemData(wrap);const copy=quoteItemRow(d);wrap.after(copy);renumberQuoteItems();calcTotal()};
 ...
 wrap.addEventListener('dragstart',e=>{wrap.classList.add('is-dragging');e.dataTransfer.effectAllowed='move'});
 wrap.addEventListener('dragend',()=>{wrap.classList.remove('is-dragging');document.querySelectorAll('.quote-item').forEach(x=>x.classList.remove('drag-over'));renumberQuoteItems();calcTotal()});
...
function getQuoteItemData(r){return {model:r.querySelector('.item-model').value.trim(),qty:+r.querySelector('.item-qty').value||0,unitPrice:+r.querySelector('.item-price').value||0,discount:+r.querySelector('.item-discount').value||0,description:r.querySelector('.item-description').value.trim(),pumpData:r.dataset.pumpData?JSON.parse(r.dataset.pumpData):null}}
...
function getQuoteItems(){return [...document.querySelectorAll('.quote-item')].map(getQuoteItemData).filter(x=>x.model||x.description||x.unitPrice||x.qty)}
...
function buildPrintQuotation(){
 ...
 $('pProject').textContent=[$('project').value,$('project2').value].filter(Boolean).join('\n')||'-';
...
function loadPrintSample(){
 ...
 $('project').value='Rawang Booster Pump Replacement';$('customerReference').value='RFQ-2026-071';$('quoteNo').value='R-2607-0180';$('preparedBy').value='Ray';window.ksSignatureImage='';$('qRevisionDate').value='';$('showRevision').checked=false;$('qDate').value=new Date().toISOString().slice(0,10);$('delivery').value='Ex - Stock subject to prior sales. Otherwise 2-3 months upon confirmation order.';$('delivery2').value='';$('validity').value='14 days';$('priceBasis').value='Ex - K.L. only, nett in Ringgit Malaysia.';$('payment').value='30 days';
...
$('project')?.addEventListener('click',()=>openTwoLineEditor('project','Project'));
