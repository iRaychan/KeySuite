(() => {
  ...
  async function loadData(){
    ...
    return {version:'1.12',release_date:'2026-07-29',currency:setting.currency||'MYR',source_currency:setting.source_currency||'USD',currency_multiplier:Number(setting.currency_multiplier||0),fuel_price:Number(setting.fuel_price??2),fuel_base_price:Number(setting.fuel_base_price??2),
  ...
  async function enter(s){
    ...
    try{
      ...
      window.KeySuitePricing?.init(data,access);window.KeySuiteRoles?.init(access);unlockSelector();
