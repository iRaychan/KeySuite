(() => {
  ...
  let secureData={companies:[],users:[],categories:[],products:[],gwsProducts:[],usd_multiplier:5.8,rmb_multiplier:.65,fuel_price:2,fuel_base_price:2};
  let access=null;
  let companyId='';
  let categoryId='';
  let visibleRows=[];
  ...
  function multipliers(){return {USD:Number(secureData.usd_multiplier??5.8),RMB:Number(secureData.rmb_multiplier??.65),MYR:1}}

  function normalizeRule(raw={},family='CHC',cat=null){
    const fallback={margin:.38,transport:30,commission:.03,setDiscount:.068,finalDiscount:.08,includeCommission:true,includeSetDiscount:true,includeFinalDiscount:true,includeFuelCharge:true};
    return {
  ...
  function formula(cat=category(),family='CHC'){
    ...
    return `${family} Final Price = ${parts.join(' → ')}`;
  }
  ...
  function currencyCandidates(priceBook={},variant){
    const rates=multipliers();
  ...
  function calculatePrice(priceBook,variant,cat=category(),family='CHC',options={}){
    if(!cat)return null;
    const candidates=currencyCandidates(priceBook,variant);
    ...
    const withTransport=marginPrice+rule.transport;
    ...
    return {
      family,variant,candidates,sourceCurrency:chosen.currency,sourcePrice:chosen.sourcePrice,multiplier:chosen.multiplier,baseMyr:chosen.baseMyr,
      margin:rule.margin,transport:rule.transport,commission:rule.commission,setDiscount:rule.setDiscount,finalDiscount:rule.finalDiscount,
      ...
      marginPrice,withTransport,afterCommission,afterSetDiscount,beforeFuel,
  ...
  function variants(includeUnpriced=false){
    ...
    for(const product of secureData.products||[]){
      for(const material of ['CHC','CHCS','CHCN']){
        const candidates=currencyCandidates(product.pricesByCurrency||{},material);
        if(candidates.length||includeUnpriced)result.push({product,material,priced:!!candidates.length});
      }
    }
    return result;
  }
  ...
  function ruleSummary(cat,family){
    const rule=categoryRule(cat,family);
    return [
      [`${family} Margin`,percent(rule.margin)],['Transport',cash(rule.transport)],
  ...
  function renderSummary(){
    ...
    const currencyRows=[['Price List Currency',`USD (MYR ${n(secureData.usd_multiplier,2)}) · RMB (MYR ${n(secureData.rmb_multiplier,2)}) · MYR (MYR 1.00)`],['Current Fuel Price',`${cash(ctx.fuelPrice)}/L`],['Customer Distance',`${n(ctx.distanceKm,1)} km`]];
    byId('pricingCategorySummary').innerHTML=cat?[['Category Name',cat.name],...ruleSummary(cat,'CHC'),...ruleSummary(cat,'GWS'),...currencyRows].map(([k,v])=>`<div class="pricing-kv"><b>${e(k)}</b><span>${e(v)}</span></div>`).join(''):'<p class="muted">No pricing category is assigned to this customer.</p>';
    byId('pricingFormula').textContent=cat?`${formula(cat,'CHC')}\n${formula(cat,'GWS')}`:'';
  ...
  function findPrice(model,options={}){
    ...
    const calc=calculatePrice(product.pricesByCurrency||{},material,cat,'CHC',{...options,customer});
    return calc?{product,material,calc,category:cat,customer,family:'CHC'}:null;
  }
  ...
  function findGwsPrice(model,pressure,options={}){
    ...
    const calc=calculatePrice(product.pricesByCurrency||{},variant,cat,'GWS',{...options,customer});
    return calc?{product,material:variant,variant,calc,category:cat,customer,family:'GWS'}:null;
  }

  function sourceSnapshot(found){
    return {product_family:found.family||found.calc.family||'CHC',product_id:found.product.id,material:found.material,variant:found.variant||found.material,customer_id:found.customer?.id||'',category_id:found.category?.id||'',source_currency:found.calc.sourceCurrency,currency_multiplier:found.calc.multiplier,source_price:found.calc.sourcePrice,distance_km:found.calc.distanceKm,fuel_price:found.calc.fuelPrice,fuel_base_price:found.calc.fuelBasePrice,fuel_charge:found.calc.fuelCharge,unrounded_price:found.calc.unroundedPrice,calculated_price:found.calc.finalPrice};
  }
  ...
  function addToQuotation(index){
    ...
    const calc=calculatePrice(row.product.pricesByCurrency||{},row.material,cat,'CHC',{customer});if(!calc)return;
  ...
  function exportCsv(){
    const rows=[['Customer','Category','Model','Variant','Source Currency','Source Price','Base MYR','Margin','Transport','Fuel Charge','Final Price']],c=company(),cat=category();
    for(const row of visibleRows){const calc=c&&cat?calculatePrice(row.product.pricesByCurrency||{},row.material,cat,'CHC',{customer:c}):null;if(calc)rows.push([c.company,cat.name,row.product.model,row.material,calc.sourceCurrency,calc.sourcePrice,calc.baseMyr,calc.margin,calc.transport,calc.fuelCharge,calc.finalPrice])}
    const csv=rows.map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\r\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));link.download='KeySuite_V1.18_Visible_Pricing.csv';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000);
  }
