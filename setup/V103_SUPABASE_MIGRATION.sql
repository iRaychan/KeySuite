-- KeySuite V1.03 secure customer ownership and role access
-- Run once in Supabase SQL Editor before testing the V1.03 Customer page.
begin;
...
alter table public.ks_customers add column if not exists classification text not null default 'Other';
...
alter table public.ks_customers add column if not exists contacts jsonb not null default '[]'::jsonb;
alter table public.ks_customers add column if not exists status text not null default 'active';
alter table public.ks_customers add column if not exists created_at timestamptz not null default now();
alter table public.ks_customers add column if not exists updated_at timestamptz not null default now();
